...
int getDistance() {
  ...
  // ############################################################
  //  ANSWER (CHALLENGE 7): duration * SPEED OF SOUND (0.034),
  //  DIVIDED BY 2 (THERE AND BACK)
  //  Taught: Day4.pptx, Slide 31
  //  ("Adding Sensors to Your Prototype")
  // ############################################################
...
void setup()
{
  ...
  // ############################################################
  //  ANSWER (CHALLENGE 6): SAME SHAPE AS THE while LOOP JUST
  //  ABOVE, RENUMBERED FOR JOYSTICK 4
  //  Taught: built from Day2.pptx, Slides 54-57
  //  (while loop) + Day3.pptx, Slides 37-38
  //  (LCD library)
  // ############################################################
...
void loop()
{
  ...
  joy4_btn = digitalRead(JOY4_BUTTON_PIN); 
   ...
   if (abs(joy4_x) > DEAD_ZONE) {
      ...
      //  (BOTH USE "-")
      //  Taught: Day3.pptx, Slide 32 ("Step 3: Each
      //  Joystick Owns a Servo" -- shows "Joy 4 X -> [1] Wrist")
      // ############################################################
      pos[1] = pos[1] - (joy4_x/speed_divider) ;
      if (pos[1] > max_pos[1]) {
          pos[1] = max_pos[1];
      }
      if (pos[1] < min_pos[1]) {
   ...
   if (joy4_btn == 0) {
       ...
       //  Taught: Day2.pptx, Slide 57 ("Step 3:
       //  Stabilize Button")
       // ############################################################
       while (joy4_btn == 0) {
           //Don't do anything until they let go }
